--- Acceptance suite for the Python "empty" service archetype - a platform overlay. Unlike the
...
local project = prova.fixture("python-empty:project", Scope.Suite, function(ctx)
...
prova.group("python-empty overlay", function(g)
  ...
  g:test("leaves no unrendered template markers", function(t)
    -- No bare {{ }}, {% %}, or {# #} in contents or path segments (GitHub Actions ${{ … }}
    -- expressions are excluded by the matcher).
